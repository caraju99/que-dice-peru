...
export default function AdminPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/markets', {
      ...
      body: JSON.stringify({ title, category, emoji, probability, closesAt })
    });
  ...
  return (
    <div className="p-4">
      ...
      <h2 className="mb-2.5 mt-6 font-display text-base font-bold text-brand-text">Mercados</h2>
            ...
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-brand-text2">
                  Probabilidad: {m.probability}% · Volumen: {m.volume.toLocaleString()} DICE · Cierra:{' '}
                  {new Date(m.closesAt).toLocaleDateString('es-PE')}
                </p>
              </div>
